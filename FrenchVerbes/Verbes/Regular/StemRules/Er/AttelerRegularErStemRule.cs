...
public sealed class AttelerRegularErStemRule : IRegularErStemRule
{
    public string Infinitive => Constants.Verbs.Regular.Er.Atteler;

    public string Apply(string stem, string tense, int pronounIndex)
    {
        bool doubleL = tense switch
        {
            Constants.Tenses.Present => pronounIndex != 3,
            Constants.Tenses.FuturSimple => true,
            Constants.Tenses.ConditionnelPresent => true,
            Constants.Tenses.Imperative => pronounIndex == 1 || pronounIndex == 4,
            _ => false
        };
